...
/-- Barycentric criterion: if the three sub-triangles obtained by replacing
one vertex of `A B C` with `x` all have the orientation of `A B C` (here
positive), then `x` lies in the triangle. -/
...
/-- A point between `A` and `B` is in any triangle `A B C`. -/
lemma inTri_of_between (x A B C : ℝ × ℝ) (t : ℝ)
    (h0 : 0 ≤ t) (h1 : t ≤ 1) (hx : x = t • A + (1 - t) • B) :
    InTri x A B C := by
