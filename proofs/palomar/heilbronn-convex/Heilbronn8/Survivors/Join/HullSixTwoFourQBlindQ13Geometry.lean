...
import Heilbronn8.Survivors.Join.HullSixTwoFourQBlindQ13DirectAMGM
import Heilbronn8.Survivors.Join.HullSixTwoFourQBlindCases

/-!
# Geometry and table adapter for the q-blind `q = 13` chamber

This adapter wires the exact `MRRR / LMMR` table to the direct mixed-fan
q13 scalar theorems.  Besides the two transitions on the first lower fan
edge, it retains the middle vertical recurrence, the first bottom-row
recurrence, and the signed terminal recurrence.  No plateau envelope or
...
private theorem qBlind13_table00 :
    qBlind13.table 0 0 = HullSixChamberLabel.M := by decide

private theorem qBlind13_table01 :
    qBlind13.table 0 1 = HullSixChamberLabel.R := by decide

private theorem qBlind13_table02 :
    qBlind13.table 0 2 = HullSixChamberLabel.R := by decide

private theorem qBlind13_table10 :
    qBlind13.table 1 0 = HullSixChamberLabel.L := by decide

private theorem qBlind13_table11 :
    qBlind13.table 1 1 = HullSixChamberLabel.M := by decide

private theorem qBlind13_table12 :
    qBlind13.table 1 2 = HullSixChamberLabel.M := by decide

private theorem qBlind13_table13 :
    qBlind13.table 1 3 = HullSixChamberLabel.R := by decide

end HullSixTwoFourCuts

private lemma qBlind13_q_fan_base_change
...
/-- The exact `MRRR / LMMR` q13 table is impossible in a beating oriented
`2 + 4` view. -/
theorem twoFourQBlindQ13At_false
    ...
    (hTable : ∀ i j,
      HullSixChamberLabel.Holds
        (HullSixTwoFourCuts.qBlind13.table i j) (minTri cfg)
        (sig (cfg P)
          (cfg (cycle (rotation + hullSixTwoFourUpperOffset i)))
  ...
  have hTFloor : 1 ≤ T := hnorm (by simpa [T] using hM11.2)
  have hJFloor : 1 ≤ J := hnorm (by simpa [J] using hM12.2)
  have hY13 : Y13 ≤ -1 := hnormNeg (by simpa [Y13] using hR13)
  have hK1 : 1 ≤ K := by
    dsimp [K]
    linarith
  ...
  have hTBase : T = X11 + b + d := by
    dsimp [T, X11, b, d, hullSixThreeThreeUpperHeight,
      hullSixThreeThreeLowerHeight]
    rw [sig_twoFour_qBlind_base_change]
    ring
  have hTUpper : T ≤ b + d - 1 := by
    rw [hTBase]
    linarith
  have hYRecurrence : b * y = e * T - d * J := by
    dsimp [b, d, e, y, T, J,
      hullSixThreeThreeUpperHeight, hullSixThreeThreeLowerHeight]
    field_simp [hm.ne'] <;> simp only [sig] <;> ring
  have hYUpper : b * y ≤ e * (b + d - 1) - d := by
    have heT : e * T ≤ e * (b + d - 1) :=
      mul_le_mul_of_nonneg_left hTUpper
        (le_trans zero_le_one he1)
    have hdJ : d ≤ d * J := by
      simpa using mul_le_mul_of_nonneg_left hJFloor
        (le_trans zero_le_one hd1)
    rw [hYRecurrence]
    linarith
  ...
  have hTerminal : b * Fp = f * (A + a - b) + a * K := by
    dsimp [a, b, f, A, Fp, K, Y13,
      hullSixThreeThreeUpperHeight, hullSixThreeThreeLowerHeight]
    field_simp [hm.ne'] <;> simp only [sig] <;> ring
  ...
  have hScalar :
    ...
    · have hcd : c < d := by
        ...
        nlinarith
      exact hullSixTwoFourQ13AscendingDirect_scalar
        ha1 hb1 hc1 hf1 hcd hde hC1 hz1 hFp1 hK1
        hAtransition hTopTransition hBottomTransition hYUpper
        hTerminal hEar0 hEar1Mixed
    · have hed : e < d := lt_of_not_ge hde
      exact hullSixTwoFourQ13DescendingDirect_scalar
        ha1 hb1 hc1 hd1 he1 hf1 hC1 hy1 hz1 hK1 hed
        hAtransition hVerticalTransition hTopTransition hBottomTransition
        hTerminal hEar1
  ...
  have hchange := qBlind13_q_fan_base_change
