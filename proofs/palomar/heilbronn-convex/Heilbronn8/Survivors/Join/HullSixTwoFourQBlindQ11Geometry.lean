...
import Heilbronn8.Survivors.Join.HullSixTwoFourQBlindQ11ShiftedLeftScalar
import Heilbronn8.Survivors.Join.HullSixTwoFourQBlindCases

/-!
# Geometry and table adapter for the q-blind `q = 11` chamber

The exact table is `MRRR / LRRR`.  Its first bottom-row `R` cell strengthens
the first primed lower-edge recurrence to

`d * (b + c + 1) + c <= b * x`.

Together with the two lower hull ears, this is precisely the input of the
shifted-left q11 scalar theorem.
-/
...
private theorem qBlind11_table00 :
    qBlind11.table 0 0 = HullSixChamberLabel.M := by decide

private theorem qBlind11_table11 :
    qBlind11.table 1 1 = HullSixChamberLabel.R := by decide

end HullSixTwoFourCuts

private lemma qBlind11_q_fan_base_change
...
/-- The exact `MRRR / LRRR` q11 table is impossible in a beating oriented
`2 + 4` view. -/
theorem twoFourQBlindQ11At_false
    ...
    (hTable : ∀ i j,
      HullSixChamberLabel.Holds
        (HullSixTwoFourCuts.qBlind11.table i j) (minTri cfg)
        (sig (cfg P)
          (cfg (cycle (rotation + hullSixTwoFourUpperOffset i)))
  ...
  have hM00 :
    ...
    simpa [m, U, L, HullSixTwoFourCuts.qBlind11_table00,
      HullSixChamberLabel.Holds] using
        hTable (0 : Fin 2) (0 : Fin 4)
  have hR11 : sig (cfg Q) (U 1) (L 1) ≤ -m := by
    simpa [m, U, L, HullSixTwoFourCuts.qBlind11_table11,
      HullSixChamberLabel.Holds] using
        hTable (1 : Fin 2) (1 : Fin 4)
  have hX00 : X00 ≤ -1 := by
    exact hnormNeg (by simpa [X00] using hM00.1)
  have hY11 : Y11 ≤ -1 := by
    exact hnormNeg (by simpa [Y11] using hR11)
  ...
  have hxIdentity : b * x + c * Y11 = d * (b + c + C) := by
    dsimp [b, c, d, C, x, Y11,
      hullSixThreeThreeUpperHeight, hullSixThreeThreeLowerHeight]
    field_simp [hm.ne'] <;> simp only [sig] <;> ring
  have hdC : d * (b + c + 1) ≤ d * (b + c + C) := by
    exact mul_le_mul_of_nonneg_left (by linarith)
      (le_trans zero_le_one hd1)
  have hcY11 : c * Y11 ≤ -c := by
    have h := mul_le_mul_of_nonneg_left hY11
      (le_trans zero_le_one hc1)
    simpa using h
  have hxTransition : d * (b + c + 1) + c ≤ b * x := by
    linarith [hxIdentity, hdC, hcY11]
  ...
  have hScalar := Heilbronn8.hullSixTwoFourQBlindQ11ShiftedLeft_scalar
    ha1 hb1 hc1 hd1 he1 hf1 hx1 hy1 hz1 hC1 hFp1
    hAtransition hxTransition hEar0 hEar1
  ...
  have hchange := qBlind11_q_fan_base_change
