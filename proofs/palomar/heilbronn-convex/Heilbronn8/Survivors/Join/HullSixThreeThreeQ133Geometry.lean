...
import Heilbronn8.Survivors.Join.HullSixThreeThreeQ133ScalarAMGM
import Heilbronn8.Survivors.Join.HullSixThreeThreeQBlindGeometryIdentities

/-!
# Geometric adapter for the q-blind `011 / 133` chamber

At a rotated oriented `3 + 3` frame, the chamber supplies the four signed
cross floors `X10`, `-X21`, `-Y01`, and `Y12`.  Boundary and line-level
floors supply the remaining hypotheses of `hullSixThreeThreeQ133_scalar`.
The normalized determinant-coordinate identities then identify its strict
conclusion with the six-term `P`-fan sum, contradicting the beating margin.
-/
...
private lemma threeThree_qBlind_shiftedBoundary_floor
...
/-- The four signed cells used by the q-blind `011 / 133` scalar
certificate are incompatible with a beating compact residual. -/
theorem threeThreeQ133At_false_of_crossFloors
    {cfg : Configuration} {cycle : Fin 6 → Fin 8} {p q P Q : Fin 8}
    (R : HullSixCompactCrossChordResidual cfg cycle p q)
    (V : HullSixOrientedView cfg cycle P Q)
    (rotation : Fin 6)
  ...
  have htRaw : m ≤ sig (cfg P) U1 U2 := by
    simpa [m, U1, U2, hullSixThreeThreeUpperOffset] using
      threeThree_qBlind_shiftedBoundary_floor R V P V.P_outside
        V.P_boundary_pos rotation 1
  have hl1qRaw : m ≤ sig (cfg Q) L1 L2 := by
    simpa [m, L1, L2, hullSixThreeThreeLowerOffset] using
      threeThree_qBlind_shiftedBoundary_floor R V Q V.Q_outside
  ...
  have hY12pos : 1 ≤ b + f - b * f * (t + w + z) := by
    rw [hY12Eq]
    exact hnorm (by simpa [m, U1, L2] using hY12)
  ...
  have hscalar :
      (25 : ℝ) / 2 <
        a * b * (r + s) + b * c * t + c * d * (s + t) +
          d * e * (s + t + w) + e * f * z +
            a * f * (r + s + t + w + z) :=
    hullSixThreeThreeQ133_scalar ha hb hc hd he hf ha1 hd1
      hs ht hw hl1q hY01neg hY12pos
...
/-- The signed q-blind branch `p = 011`, `q = 133`. -/
def q133 : HullSixThreeThreeCuts where
  p0 := 0
  p1 := 1
  p2 := 1
  q0 := 1
...
/-- Ferrers-table wrapper for `threeThreeQ133At_false_of_crossFloors`. -/
theorem threeThreeQ133At_false
    {cfg : Configuration} {cycle : Fin 6 → Fin 8} {p q P Q : Fin 8}
    (R : HullSixCompactCrossChordResidual cfg cycle p q)
    (V : HullSixOrientedView cfg cycle P Q)
    (rotation : Fin 6)
    ...
    (hQ133 : ∀ i j : Fin 3,
      HullSixChamberLabel.Holds
        (HullSixThreeThreeCuts.q133.table i j) (minTri cfg)
        (sig (cfg P)
          (cfg (cycle (rotation + hullSixThreeThreeUpperOffset i)))
  ...
  have h01 := hQ133 0 1
  have h12 := hQ133 1 2
  rw [show HullSixThreeThreeCuts.q133.table 1 0 =
    HullSixChamberLabel.L by decide] at h10
  rw [show HullSixThreeThreeCuts.q133.table 2 1 =
    HullSixChamberLabel.M by decide] at h21
  rw [show HullSixThreeThreeCuts.q133.table 0 1 =
    HullSixChamberLabel.R by decide] at h01
  rw [show HullSixThreeThreeCuts.q133.table 1 2 =
