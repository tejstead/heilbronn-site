...
import Heilbronn8.Survivors.Join.HullSixTwoFourQBlindQ33ShiftedRightScalar
import Heilbronn8.Survivors.Join.HullSixTwoFourQBlindCases

/-!
# Geometry and table adapter for the q-blind `q = 33` chamber

The exact table is `MMMR / LMMR`.  Its first lower `P`-fan edge is the
shifted quantity `x + c - d`, while its final top-row transition gives
`e + f <= a * z`.  These bounds and the two lower hull ears are exactly the
input of the shifted-right q33 scalar theorem.
-/
...
private theorem qBlind33_table00 :
    qBlind33.table 0 0 = HullSixChamberLabel.M := by decide

private theorem qBlind33_table02 :
    qBlind33.table 0 2 = HullSixChamberLabel.M := by decide

private theorem qBlind33_table03 :
    qBlind33.table 0 3 = HullSixChamberLabel.R := by decide

private theorem qBlind33_table11 :
    qBlind33.table 1 1 = HullSixChamberLabel.M := by decide

end HullSixTwoFourCuts

private lemma qBlind33_q_fan_base_change_first_two
...
/-- The exact `MMMR / LMMR` q33 table is impossible in a beating oriented
`2 + 4` view. -/
theorem twoFourQBlindQ33At_false
    ...
    (hTable : ∀ i j,
      HullSixChamberLabel.Holds
        (HullSixTwoFourCuts.qBlind33.table i j) (minTri cfg)
        (sig (cfg P)
          (cfg (cycle (rotation + hullSixTwoFourUpperOffset i)))
  ...
  have hCfan1 : 1 ≤ Cfan := by
    have h := R.twoFour_P_boundary_floor V rotation 1
    exact hnorm (by simpa [m, Cfan, U, L,
  ...
  have hM00 :
    ...
    simpa [m, U, L, HullSixTwoFourCuts.qBlind33_table00,
      HullSixChamberLabel.Holds] using
        hTable (0 : Fin 2) (0 : Fin 4)
  have hM02 :
      sig (cfg P) (U 0) (L 2) ≤ -m ∧
        m ≤ sig (cfg Q) (U 0) (L 2) := by
    simpa [m, U, L, HullSixTwoFourCuts.qBlind33_table02,
      HullSixChamberLabel.Holds] using
        hTable (0 : Fin 2) (2 : Fin 4)
  have hR03 : sig (cfg Q) (U 0) (L 3) ≤ -m := by
    simpa [m, U, L, HullSixTwoFourCuts.qBlind33_table03,
      HullSixChamberLabel.Holds] using
        hTable (0 : Fin 2) (3 : Fin 4)
  have hM11 :
      sig (cfg P) (U 1) (L 1) ≤ -m ∧
        m ≤ sig (cfg Q) (U 1) (L 1) := by
    simpa [m, U, L, HullSixTwoFourCuts.qBlind33_table11,
      HullSixChamberLabel.Holds] using
  ...
  have hAIdentity : c * A = a * Cfan - b * X00 := by
    dsimp [a, b, c, A, Cfan, X00,
      hullSixThreeThreeUpperHeight, hullSixThreeThreeLowerHeight]
    field_simp [hm.ne'] <;> simp only [sig] <;> ring
  have hAtransition : a + b ≤ c * A := by
    rw [hAIdentity]
    have haCfan : a ≤ a * Cfan := by
      simpa using mul_le_mul_of_nonneg_left hCfan1
  ...
  have hE0Base : x = E0 - c + d := by
    ...
    ring
  have hxP1 : 1 ≤ x + c - d := by
    have hshape : E0 = x + c - d := by linarith [hE0Base]
    rw [← hshape]
    exact hE01
  have hE0Identity : b * E0 = d * Cfan - c * X11 := by
    dsimp [b, c, d, E0, Cfan, X11,
      hullSixThreeThreeUpperHeight, hullSixThreeThreeLowerHeight]
    field_simp [hm.ne'] <;> simp only [sig] <;> ring
  have hxTransitionE0 : c + d ≤ b * E0 := by
    rw [hE0Identity]
    have hdCfan : d ≤ d * Cfan := by
      simpa using mul_le_mul_of_nonneg_left hCfan1
    ...
    linarith
  have hxTransition : c + d ≤ b * (x + c - d) := by
    have hshape : E0 = x + c - d := by linarith [hE0Base]
    rwa [hshape] at hxTransitionE0

  have hzIdentity : a * z = f * Y02 - e * Y03 := by
    dsimp [a, e, f, z, Y02, Y03,
      hullSixThreeThreeUpperHeight, hullSixThreeThreeLowerHeight]
    field_simp [hm.ne'] <;> simp only [sig] <;> ring
  have hzTransition : e + f ≤ a * z := by
    rw [hzIdentity]
    have hfY : f ≤ f * Y02 := by
  ...
  have hScalar := Heilbronn8.hullSixTwoFourQ33ShiftedRight_scalar
    ha1 hb1 hc1 hd1 he1 hf1 hCfan1 hx1 hy1 hz1 hFp1 hxP1
    hAtransition hxTransition hzTransition hEar0 hEar1
