...
import Heil7.Hull7ContinuantStrict

/-!
# Strict direct hull-seven route through fan minors
...
The conclusion is the strict canonical hull-seven triangle bound when the minimum is positive.  A separate lemma
...
theorem hullSeven_continuant_strict :
    ∀ (v : Configuration7), HullCCW v 7 → 0 < minTri v →
      9 * minTri v < fanArea v 7 := by
  intro v hull hmGiven
  ...
  · linarith
  · have hscalar := nine_mul_lt_fanSum_of_hullSeven_continuant
