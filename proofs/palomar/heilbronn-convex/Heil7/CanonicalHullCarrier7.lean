import Heil7.CanonicalUpper
import Mathlib.Analysis.Convex.Independent
import Mathlib.Analysis.Convex.KreinMilman

/-!
# A finite irredundant carrier for a seven-point hull

On the positive-`minTri` branch every three distinct labelled points are
noncollinear.  The extreme points of the finite convex hull can therefore be
pulled back to an injectively labelled finite carrier.  Krein--Milman has no
closure cost because that extreme-point set is finite.

This is the `Fin 7` specialization of the certificate-free construction used
by the n=8 hull-cycle development.  It depends only on Mathlib finite convex
geometry and the canonical Palomar definitions.
-/

set_option linter.style.header false

namespace HeilbronnChallenge.N7Upper

open Set

/-- General position in the arbitrary-label order needed by the hull carrier
and cyclic-order arguments. -/
def AllTripleNonzero7 (v : Configuration7) : Prop :=
  ∀ a b c : Fin 7, a ≠ b → a ≠ c → b ≠ c →
    sig (v a) (v b) (v c) ≠ 0

/-- A positive canonical minimum excludes every collinear distinct triple. -/
lemma allTripleNonzero7_of_minTri_pos (v : Configuration7)
    (hm : 0 < minTri v) : AllTripleNonzero7 v := by
  intro a b c hab hac hbc hzero
  have hle := minTri_le_of_distinct v a b c hab hac hbc
  rw [hzero, abs_zero] at hle
  linarith

lemma AllTripleNonzero7.injective {v : Configuration7}
    (h : AllTripleNonzero7 v) : Function.Injective v := by
  intro i j hij
  by_contra hne
  obtain ⟨k, hki, hkj⟩ :=
    Fin.exists_ne_and_ne_of_two_lt i j (by decide)
  have hs := h i j k hne hki.symm hkj.symm
  apply hs
  rw [hij]
  simp [sig]

/-- Extreme labels carrying the whole convex hull, with no redundant carrier
vertex. -/
structure StrictHullCarrier7 (v : Configuration7) where
  labels : Finset (Fin 7)
  card_ge_three : 3 ≤ labels.card
  card_le_seven : labels.card ≤ 7
  covers : ∀ p : Fin 7,
    v p ∈ convexHull ℝ (v '' (↑labels : Set (Fin 7)))
  irredundant : ∀ q ∈ labels,
    v q ∉ convexHull ℝ (v '' (↑(labels.erase q) : Set (Fin 7)))

private lemma sig_eq_zero_of_collinear_triple7
    {a b c : ℝ × ℝ}
    (h : Collinear ℝ ({a, b, c} : Set (ℝ × ℝ))) :
    sig a b c = 0 := by
  rw [collinear_iff_exists_forall_eq_smul_vadd] at h
  obtain ⟨p, w, hw⟩ := h
  obtain ⟨ra, hra⟩ := hw a (by simp)
  obtain ⟨rb, hrb⟩ := hw b (by simp)
  obtain ⟨rc, hrc⟩ := hw c (by simp)
  rw [hra, hrb, hrc]
  simp [sig]
  ring

/-- Every seven-point configuration in general position has a finite
convex-independent label carrier of size between three and seven. -/
noncomputable def strictHullCarrier7_exists
    (v : Configuration7) (hstrict : AllTripleNonzero7 v) :
    StrictHullCarrier7 v := by
  classical
  let K : Set (ℝ × ℝ) := convexHull ℝ (Set.range v)
  let E : Set (ℝ × ℝ) := K.extremePoints ℝ
  let labels : Finset (Fin 7) :=
    Finset.univ.filter fun i ↦ v i ∈ E

  have hKcompact : IsCompact K := by
    simpa [K] using HullBridge.isCompact_convexHull_range v
  have hKconvex : Convex ℝ K := by
    simpa [K] using convex_convexHull ℝ (Set.range v)
  have hEsubset : E ⊆ Set.range v := by
    simpa [E, K] using
      (extremePoints_convexHull_subset
        (𝕜 := ℝ) (A := Set.range v))
  have hEfinite : E.Finite :=
    (Set.finite_range v).subset hEsubset
  have hEK : convexHull ℝ E = K := by
    have hKM := closure_convexHull_extremePoints hKcompact hKconvex
    have hclosed : IsClosed (convexHull ℝ E) :=
      hEfinite.isClosed_convexHull ℝ
    rw [hclosed.closure_eq] at hKM
    exact hKM
  have himage : v '' (↑labels : Set (Fin 7)) = E := by
    apply Set.Subset.antisymm
    · rintro _ ⟨i, hi, rfl⟩
      simpa [labels] using hi
    · intro x hx
      obtain ⟨i, rfl⟩ := hEsubset hx
      exact ⟨i, by simpa [labels] using hx, rfl⟩
  have hcover : ∀ p : Fin 7,
      v p ∈ convexHull ℝ (v '' (↑labels : Set (Fin 7))) := by
    intro p
    rw [himage, hEK]
    exact subset_convexHull ℝ (Set.range v) ⟨p, rfl⟩

  have hvInjective : Function.Injective v := hstrict.injective
  have hEindependent : ConvexIndependent ℝ ((↑) : E → ℝ × ℝ) :=
    hKconvex.convexIndependent_extremePoints
  have hEirredundant : ∀ x ∈ E,
      x ∉ convexHull ℝ (E \ {x}) :=
    convexIndependent_set_iff_notMem_convexHull_sdiff.mp hEindependent
  have hirredundant : ∀ q ∈ labels,
      v q ∉ convexHull ℝ
        (v '' (↑(labels.erase q) : Set (Fin 7))) := by
    intro q hq hbad
    have hvqE : v q ∈ E := by simpa [labels] using hq
    apply hEirredundant (v q) hvqE
    apply (convexHull_mono ?_) hbad
    rintro _ ⟨i, hi, rfl⟩
    have hiErase : i ∈ labels.erase q := by
      simpa [Finset.mem_erase, and_comm] using hi
    have hiLabels : i ∈ labels := Finset.mem_of_mem_erase hiErase
    have hiq : i ≠ q := Finset.ne_of_mem_erase hiErase
    refine ⟨?_, ?_⟩
    · simpa [labels] using hiLabels
    · intro heq
      exact hiq (hvInjective heq)

  have hcard_le : labels.card ≤ 7 := by
    simpa using Finset.card_le_univ labels
  have hcard_ge : 3 ≤ labels.card := by
    by_contra hnot
    have hle : labels.card ≤ 2 := by omega
    have hsizes : labels.card = 0 ∨ labels.card = 1 ∨
        labels.card = 2 := by omega
    have h012 : sig (v 0) (v 1) (v 2) ≠ 0 :=
      hstrict 0 1 2 (by decide) (by decide) (by decide)
    rcases hsizes with hzero | hone | htwo
    · have hlabels : labels = ∅ := Finset.card_eq_zero.mp hzero
      have h := hcover 0
      simp [hlabels] at h
    · obtain ⟨a, ha⟩ := Finset.card_eq_one.mp hone
      have h0 : v 0 = v a := by
        have := hcover 0
        simpa [ha] using this
      have h1 : v 1 = v a := by
        have := hcover 1
        simpa [ha] using this
      apply h012
      rw [h0, h1]
      simp [sig]
    · obtain ⟨a, b, hab, habLabels⟩ := Finset.card_eq_two.mp htwo
      have hline : ∀ p : Fin 7,
          v p ∈ affineSpan ℝ ({v a, v b} : Set (ℝ × ℝ)) := by
        intro p
        apply convexHull_subset_affineSpan (𝕜 := ℝ)
          ({v a, v b} : Set (ℝ × ℝ))
        have := hcover p
        have himage' :
            v '' (↑labels : Set (Fin 7)) =
              ({v a, v b} : Set (ℝ × ℝ)) := by
          ext z
          simp [habLabels, eq_comm]
        rw [himage'] at this
        simpa only [convexHull_pair] using this
      have hcol : Collinear ℝ ({v 0, v 1, v 2} : Set (ℝ × ℝ)) :=
        collinear_triple_of_mem_affineSpan_pair
          (hline 0) (hline 1) (hline 2)
      exact h012 (sig_eq_zero_of_collinear_triple7 hcol)
  exact ⟨labels, hcard_ge, hcard_le, hcover, hirredundant⟩

end HeilbronnChallenge.N7Upper
